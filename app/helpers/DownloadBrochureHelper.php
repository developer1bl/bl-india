...
class DownloadBrochureHelper
{

    public static function getSectionData(int $num)
    {
        if ($num) {
    ...
    public static function getCountryData()
    {
        ...
        $country = Countries::all(['name', 'phonecode']);
        ...
        foreach ($data as $country) {
            $transformedData[] = [
                'countryName' => $country['name'] . ' (+' . $country['phonecode'] . ')',
                'countryData' => json_encode([$country['name'], $country['phonecode']])
            ];
        }
    ...
    public static function SendThanksMail($data)
    {
        if (!empty($data['email'])) {
            ...
            $thanks = "<p style='font-family: Arial, Helvetica, sans-serif; font-size: 18px; color: #000;'>Hello " . $data['name'] . ",<br/>" .
                "Thank you for downloading our brochure for <b>" . $data['service_name'] . "</b>!</p>" .
            ...
            Mail::to($data['email'])->send(new ThanksMail($data1));
        }
    }
}
